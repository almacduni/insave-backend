...
public class GuppyMultipleMovingAverageIndicator extends CachedIndicator<MultipleMovingAverage> {
  ...
  public MultipleMovingAverage calculate(int index) {
    ...
    if (index > Period.TWELVE.getTimeFrame() - 1)
      gmma.setValue(Period.TWELVE, ema12.getValue(index));
    if (index > Period.FIFTEEN.getTimeFrame() - 1)
      gmma.setValue(Period.FIFTEEN, ema15.getValue(index));

    // Long term moving averages
    if (index > Period.THIRTY.getTimeFrame() - 1)
      gmma.setValue(Period.THIRTY, ema30.getValue(index));
    if (index > Period.THIRTYFIVE.getTimeFrame() - 1)
      gmma.setValue(Period.THIRTYFIVE, ema35.getValue(index));
    if (index > Period.FORTY.getTimeFrame() - 1) gmma.setValue(Period.FORTY, ema40.getValue(index));
    if (index > Period.FORTYFIVE.getTimeFrame() - 1)
      gmma.setValue(Period.FORTYFIVE, ema45.getValue(index));
